...
- `docs/roadmap.md` ([file](../roadmap.md)) — Layer-by-layer release status. v0.1.0 shipped Layer 1 (boundary). Layer 2 (agent primitives) in progress; v0.2.0 targets Layer 2 complete plus MCP outbound bridge and UI bridge.
- `docs/contributing.md` ([file](../contributing.md)) — Dev setup (Go 1.25+, `go test ./...`), OpenSpec workflow, design-spec/execution-plan locations.
...
- **Layer-status tracking** lives in active OpenSpec changes. (roadmap.md)
...
4. **`AGENTS.md` and `CLAUDE.md` at the repo root** are referenced by `contributing.md` for the OpenSpec workflow but the contents are not summarized; reviewer should skim those once.
